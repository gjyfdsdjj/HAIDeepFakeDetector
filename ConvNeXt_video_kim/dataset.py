...
class DeepfakeDataset(Dataset):
    ...
    def __init__(
        ...
        if video_real_path.exists():
            ...
            if self.max_video_samples_per_class:
                ...
                start_idx = self.sample_offset
                end_idx = start_idx + self.max_video_samples_per_class
                video_real_files = video_real_files[start_idx:end_idx]  # ← config.py의 max_video_samples_per_class 값 사용
        ...
        if video_fake_path.exists():
            ...
            if self.max_video_samples_per_class:
                ...
                start_idx = self.sample_offset
                end_idx = start_idx + self.max_video_samples_per_class
                video_fake_files = video_fake_files[start_idx:end_idx]  # ← config.py의 max_video_samples_per_class 값 사용
    ...
    def __getitem__(self, idx):
        ...
        try:
            if file_type == 'video':
                ...
                if len(frames) == 0:
                    ...
                    # [1, C, H, W]
                    video_tensor = image.unsqueeze(0)
                else:
                    processed = []
                    for fr in frames:
                        fr = self.process_image(fr)
                        if self.transform:
                            fr = self.transform(image=fr)["image"]   # [C,H,W]
                        else:
                            fr = torch.from_numpy(fr).permute(2,0,1).float()/255.0
                        processed.append(fr)
                    # [N, C, H, W]
        ...
        if self.num_frames_per_video > 1:
            image = image.repeat(self.num_frames_per_video, 1, 1, 1)  # [N,C,H,W]
